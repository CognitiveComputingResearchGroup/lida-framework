 ...
 * <b>class name</b> - canonical name of the panel's java class, must implement {@link GuiPanel}<br/>
...
public class FrameworkGui extends javax.swing.JFrame implements FrameworkGuiEventListener {
    ...
    /**
     * Constructs a new FrameworkGui using the {@link Agent} object as the model and {@link FrameworkGuiController} as
     * the controller. Reads the {@link Properties} file and creates and configures the {@link GuiPanel}s specified
     * therein.
     * 
     * @param a
     ...
     * @param config
     *            {@link FrameworkGuiDef} for configuration
     */
    public FrameworkGui(Agent a, FrameworkGuiController cont, FrameworkGuiDef config) {
        initComponents();
        if (a == null) {
     ...
     * 
     * @param config
     *            Parameters specified in 'configs.guiPanels.properties' (or whatever file name is given by the
     *            lida.gui.panels property)
     */
    ...
    private void addGuiPanel(GuiPanel panel, String panelPosition) {
        ...
        if ("A".equalsIgnoreCase(panelPosition)) {
            ...
            logger.log(Level.WARNING, "Position error for panel " + panel.getName() + " pos:" + panelPosition, 0L);
        }
    ...
    private void initComponents() {
        ...
        jSplitPane1
                .setDividerLocation(GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().height / 2);
        jSplitPane1.setOrientation(javax.swing.JSplitPane.VERTICAL_SPLIT);

        jSplitPane2
                .setDividerLocation(GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().width / 2);
    ...
    private void guiRefreshRateMunuItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guiRefreshRateMunuItemActionPerformed
        ...
        String sticks = JOptionPane.showInputDialog("Enter a new gui refresh rate (zero is no refresh):",
                guiRefreshInterval);
        if (sticks != null) {
            try {
                ...
                JOptionPane.showMessageDialog(this,
                        "Error in the new Gui refresh interval. must be a positive int or zero.", "Error",
