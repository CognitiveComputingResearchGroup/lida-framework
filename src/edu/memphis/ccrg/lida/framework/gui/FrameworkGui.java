 ...
 * regions. Each region can contain one or more {@link GuiPanel}s specified by a
 * config file. Panels can be assigned to one of the regions or can "float"
 ...
 */
public class FrameworkGui extends javax.swing.JFrame implements
	...
	/**
	 * Constructs a new FrameworkGui using the {@link Agent} object as the model
	 * and {@link FrameworkGuiController} as the controller. Reads the
	 * {@link Properties} file and creates and configures the {@link GuiPanel}s
	 * specified therein.
	 * 
	 * @param a
	 ...
	 */
	public FrameworkGui(Agent a, FrameworkGuiController cont, Properties prop) {
		initComponents();
		if (a == null) {
			logger.log(Level.WARNING,
					"Agent is null. Panels will not be loaded.", TaskManager
							.getCurrentTick());
		} else if (cont == null) {
			logger.log(Level.WARNING,
					"Controller is null. Panels will not be loaded.",
					TaskManager.getCurrentTick());
		} else if (prop == null) {
			logger.log(Level.WARNING,
					"Properties are null. Panels will not be loaded.",
					TaskManager.getCurrentTick());
		} else {
			agent = a;
			controller = cont;
	...
	private void loadPanels(Properties panelProp) {
		...
		for (Object key : panelProp.keySet()) {
			...
			if ((vals.length < FIRST_PARAM)) {
				logger.log(Level.WARNING, "Error reading line for Panel {1}",
						new Object[] { 0L, key });
			} else {
				panelsArray[i++] = vals;
			}
		...
		if (panelParams[MUST_REFRESH].equalsIgnoreCase("Y")) {
			try {
				...
				logger.log(Level.SEVERE,
						...
						new Object[] { 0L, e, panel });
				e.printStackTrace();// TODO print stack trace in logging panel
									// also
				// logger.log(Level.SEVERE,"Exception Stack Trace",e);
			}
		}
	 ...
	 * 
	 * @param panelPosition Determines where the panel is going to be placed.
	 * <br/> A: Upper Left position. <br/> B: Upper Right position. In a new
	 * TAB<br/> C: Lower position. In a new TAB<br/> TOOL: In the ToolBox <br/>
	...
	private void addToPanelsMenu(final GuiPanel panel, final Container parent,
			JMenu associatedMenu) {
		final JPanel jPanel = panel.getPanel();
		JMenu cMenu;
		...
		// final String[] panelParams =
		// panelParameters.get(panels.indexOf(panel));
	...
	private void initComponents() {
		...
		jSplitPane1
				.setDividerLocation(GraphicsEnvironment
		...
		java.awt.Dimension screenSize = java.awt.Toolkit.getDefaultToolkit()
				.getScreenSize();
		setBounds((screenSize.width - 705) / 2, (screenSize.height - 433) / 2,
				705, 433);
		...
		JOptionPane
				.showMessageDialog(
						...
						"LIDA Framework 1.1 \n "
								...
								+ " ccrg.cs.memphis.edu", "About",
						JOptionPane.INFORMATION_MESSAGE);
	}// GEN-LAST:event_aboutMenuItemActionPerformed

	private void contentsMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_contentsMenuItemActionPerformed
		JOptionPane.showMessageDialog(this,
				"For more on the LIDA framework visit:\n"
						+ "ccrg.cs.memphis.edu/framework.html", "Help",
				JOptionPane.INFORMATION_MESSAGE);
	}// GEN-LAST:event_contentsMenuItemActionPerformed

	private void guiRefreshRateMunuItemActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_guiRefreshRateMunuItemActionPerformed
		TaskManager tm = agent.getTaskManager();
		int guiRefreshInterval = tm.getGuiEventsInterval();
		String sticks = JOptionPane.showInputDialog(
				"Enter a new gui refresh rate (zero is no refresh):",
				guiRefreshInterval);
		if (sticks != null) {
			try {
				...
				JOptionPane
						.showMessageDialog(
								this,
								"Error in the new Gui refresh interval. must be a positive int or zero.",
	...
	@SuppressWarnings("unused")
	private void loadPanelSettingsMenuItemActionPerformed(
			java.awt.event.ActionEvent evt) {
	...
	// for (java.awt.Component c : jpanel.getComponents()) {
	// if (c instanceof javax.swing.JButton) {
	// ((javax.swing.JButton) c).addActionListener(new
