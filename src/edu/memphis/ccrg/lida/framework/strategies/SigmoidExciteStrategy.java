...
/**
 * Default implementation of sigmoid excite. Uses two parameters in activation
 * calculation. Can pass these parameters when the strategy is initialized. (see
 * factoriesData.xml). Alternatively, parameters can be passed in the excite
 * method call. Formula used: 1 / (1 + exp(-a* x + c))
 * 
 * @author Ryan J. McCall
 * @author Javier Snaider
 */
public class SigmoidExciteStrategy extends StrategyImpl implements
		ExciteStrategy {
	 ...
	 * <br/>
	 * 
	 * <b>a</b> slope component of the decay function's linear scaling, 1 / (1 +
	 * exp(-a* x + c))<br/>
	 * <b>c</b> intercept component of the decay function's linear scaling, 1 /
	 * (1 + exp(-a* x + c))<br/>
	 ...
	 * @param params
	 *            optionally accepts 2 double parameters of sigmoid activation
	 *            calculation.
	 * @return new activation amount
	 */
	@Override
	public double excite(double curActiv, double excitation, Object... params) {
		double aa = a;
		double cc = c;
		if (params.length == 2) {
			aa = (Double) params[0];
	 ...
	 *            amount of activation to adds
	 * @param params
	 *            optionally accepts 2 parameters of sigmoid activation
	 *            calculation.
	 * @return new activation amount
	 */
	@Override
	public double excite(double currentActivation, double excitation,
		...
		double aa = a;
		double cc = c;
		if (params != null && params.containsKey("a")
