...
/**
 * This class allows an {@link Agent} to be started in these ways using the
 * static methods start() 1. Using the default agent properties path (a String)
 * 2. Using a agent properties path (a String) 3. Using a Properties object
 * created in another class (and/or project).
 * 
...
public class AgentStarter {
	...
	private static final Logger logger = Logger.getLogger(AgentStarter.class
			.getCanonicalName());
	...
	public static void start() {
		...
		if (agentProperties != null) {
			...
			logger
					.log(
							Level.SEVERE,
							"Could not load main configuration file from default path: {0}. Application cannot start.",
							propertiesPath);
		}
	}
	...
	/**
	 * @param path
	 *            - Properties file path used for the agent
	 */
	...
	/*
	 * Creates and run an {@link Agent} and a {@link FrameworkGuiFactory} using
	...
	private static void run() {
		...
		GlobalInitializer.getInstance().clearAttributes(); // clears Globally
															// accessible
															// initializer
															// variables
