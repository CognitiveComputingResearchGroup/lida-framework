...
public class FactoriesDataXmlLoader {
    ...
    private static StrategyFactory stratfactory = DefaultStrategyFactory.getInstance();
    private static FrameworkTaskFactory taskfactory = DefaultFrameworkTaskFactory.getInstance();
    ...
    /**
     * Parses the xml document creating the elements for factories.
     * 
     * @param dom
    ...
    static void parseDocument(Document dom) {
        ...
        // TODO: Add factory for cognitive content?
    ...
    static Map<String, LinkableDef> getLinkables(Element element, String groupName,
        ...
        List<Element> list = XmlUtils.getChildrenInGroup(element, groupName, childName);
        if (list != null && list.size() > 0) {
            for (Element e : list) {
