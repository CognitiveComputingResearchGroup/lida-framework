 ...
 * 
 */
public class WorkspaceImpl extends FrameworkModuleImpl implements WorkspaceNS, PamListener,
    ...
    public void addListener(ModuleListener l) {
        boolean unrecognizedListener = true;

        if (l instanceof WorkspaceListener) {
            addWorkspaceListener((WorkspaceListener) l);
            unrecognizedListener = false;
        }

        if (l instanceof CueListener) {
            addCueListener((CueListener) l);
            unrecognizedListener = false;
        }

        if (unrecognizedListener) {
            logger.log(Level.WARNING, "Listener {1} was not added, wrong type.", new Object[] {
    ...
    public void receiveLocalAssociation(NodeStructure association) {
        if (containsSubmodule(ModuleName.EpisodicBuffer)) {
            ...
            buffer.addBufferContent(association);
    ...
    public void receivePercept(NodeStructure newPercept) {
        if (containsSubmodule(ModuleName.PerceptualBuffer)) {
            ...
            buffer.addBufferContent(newPercept);
        } else {
