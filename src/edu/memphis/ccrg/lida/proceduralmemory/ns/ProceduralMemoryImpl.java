...
public class ProceduralMemoryImpl extends FrameworkModuleImpl implements
	 ...
	 * same java object of Node to exist in multiple places.
	 * 
	 * @author Ryan J. McCall
	...
	/*
	 * DecayStrategy used by all conditions in the condition pool (and broadcast
	 * buffer).
	 */
	 ...
	 * <br/>
	 * 
	 * <b>proceduralMemory.schemeSelectionThreshold type=double</b> amount of
	 * activation schemes must have to be instantiated, default is 0.0<br/>
	 * <b>proceduralMemory.contextWeight type=double</b> The weight of context
	 * conditions for the calculation of scheme activation. Should be positive<br/>
	 * <b>proceduralMemory.addingListWeight type=double</b> The weight of adding
	 * list conditions for the calculation of scheme activation. Should be
	 * positive<br/>
	 * <b>proceduralMemory.conditionDecayStrategy type=string</b> The
	 * DecayStrategy used by all conditions in the condition pool (and broadcast
	 * buffer).<br/>
	 * <b>proceduralMemory.schemeClass type=string</b> qualified name of the
	 * {@link Scheme} class used by this module <br/>
	 * 
	 * @see Initializable
	 */
	@Override
	...
	public void addListener(ModuleListener l) {
		if (l instanceof ProceduralMemoryListener) {
			...
			logger.log(Level.WARNING,
					"Requires ProceduralMemoryListener but received {1}",
	 ...
	 * 
	 * @param c
	 *            the condition to add to the condition pool.
	 * @return c or the stored condition with same id as c
	 */
	Condition addCondition(Condition c) {
		if (c == null) {
			logger.log(Level.WARNING, "Cannot add null condition", TaskManager
					.getCurrentTick());
			return null;
		}
		...
		if (stored == null) {
			logger.log(Level.FINEST,
					"New Condition {1} added to condition pool.", new Object[] {
	...
	/**
	 * Add a reference to specified Scheme from specified condition. Thus the
	 * presence of Condition c in the broadcast buffer will tend to activate s.
	 * The specified scheme should be one that will have c in its conditions
	 * (e.g. in the Context or adding list) Indexes Scheme s by Condition c of
	 * ConditionType type.
	 * 
	 * @param s
	 *            the {@link Scheme} to index
	 * @param c
	 *            the condition
	 * @param type
