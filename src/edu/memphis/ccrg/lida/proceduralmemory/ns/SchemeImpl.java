...
import edu.memphis.ccrg.lida.framework.shared.activation.ns.LearnableImpl;
...
public class SchemeImpl extends LearnableImpl implements Scheme {
    ...
    private final Map<Object, Condition> context = new ConcurrentHashMap<Object, Condition>();
    private final Map<Object, Condition> addingList = new ConcurrentHashMap<Object, Condition>();
    private final Map<Object, Condition> deletingList = new ConcurrentHashMap<Object, Condition>();
    ...
    /**
     * Sets the {@link ProceduralMemoryImpl} to which this {@link SchemeImpl}
     * belongs.
     * 
     * @param pm
     *            a {@link ProceduralMemoryImpl}
    ...
    /**
     * @param w
     *            the contextWeight to set
     */
    static void setContextWeight(double w) {
        if (w >= 0.0) {
            SchemeImpl.contextWeight = w;
        } else {
            logger.log(Level.WARNING, "Context weight must be positive",
    ...
    /**
     * @param w
     *            the addingListWeight to set
     */
    static void setAddingListWeight(double w) {
        if (w >= 0.0) {
            SchemeImpl.addingListWeight = w;
        } else {
            logger.log(Level.WARNING, "Adding list weight must be positive",
                    TaskManager.getCurrentTick());
        }
    }
    ...
    public double getReliability() {
        return (numExecutions > 0) ? ((double) numSuccessfulExecutions) / numExecutions : 0.0;
    }
    ...
    public boolean addCondition(Condition c, String type) {
        ...
        if (condition != null) {
            ...
            if (!wasAdded) {
                logger.log(
                        Level.WARNING,
                        "Error adding condition {1}. Condition added to ProceduralMemory but not to Scheme.",
