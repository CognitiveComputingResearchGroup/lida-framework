...
/**
 * Basic {@link ProceduralMemoryNS} {@link Initializer} which reads String
 * parameters beginning with 'scheme.' and creates a scheme based on the
 * parameter. The definition is: <br/>
 * <b>schemeLabel|(contextNode1, contextNode2,...)(contextLink1,
 * contextLink2,...)|actionName|(resultNode1, resultNode2,...)(resultLink1,
 * resultLink2,...)|baseLevelActivation</b>
 * 
...
public class BasicProceduralMemoryInitializer implements Initializer {
    ...
    private static final Logger logger = Logger
            .getLogger(BasicProceduralMemoryInitializer.class.getCanonicalName());

    @Override
    public void initModule(FullyInitializable module, Agent agent, Map<String, ?> params) {
        ProceduralMemoryNS pm = (ProceduralMemoryNS) module;
            ...
            if (key.startsWith("scheme")) {
                ...
                if (elements.length == 5) {
                    ...
                    try {
                        ...
                        logger.log(Level.WARNING, "could not parse base-level activation: {0}",
                                blActivation);
                    }
                    ...
                    for (Node n : contextNS.getNodes()) {
                        s.addCondition(n, ConditionType.CONTEXT.toString());
                    }
                    for (Node n : resultNS.getNodes()) {
                        s.addCondition(n, ConditionType.ADDINGLIST.toString());
                    }
                } else {
                    logger.log(
                            Level.WARNING,
     ...
     */
    private static NodeStructure loadNodeStructure(GlobalInitializer initializer,
            String nsSpecification) {
        String[] contextNodes = nsSpecification
                .substring(nsSpecification.indexOf('(') + 1, nsSpecification.indexOf(')'))
                .trim().split(",");
        int startLinks = nsSpecification.indexOf(')') + 1;
        String contexLinksSpec = nsSpecification.substring(startLinks);
        String[] contextLinks = contexLinksSpec
                .substring(contexLinksSpec.indexOf('(') + 1, contexLinksSpec.indexOf(')'))
                .trim().split(",");
        ...
        for (String nodeName : contextNodes) {
            ...
            if (n != null) {
                ...
                logger.log(Level.WARNING, "could not find node {0} in global initializer",
                        nodeName);
            }
        }
            ...
            if (nodes.length == 2) {
                ...
                if (source != null && sink != null) {
                    loadedNodeStructure.addDefaultLink(source, sink,
                            PerceptualAssociativeMemoryNSImpl.PARENT, 1.0, -1.0);
                } else {
                    logger.log(Level.WARNING,
                            "could not find source or sink of link {0} in global initializer",
                            linkName);
                }
            } else {
                logger.log(Level.WARNING, "bad link specification {0} ", linkName);
            }
        }
