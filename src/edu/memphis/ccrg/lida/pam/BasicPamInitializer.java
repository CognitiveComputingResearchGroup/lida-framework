...
/**
 * An {@link Initializer} for {@link PerceptualAssociativeMemory} which receives
 * a parameter named 'nodes' containing a list of node labels. Nodes are created
 * in {@link PerceptualAssociativeMemory} and nodes are added to the
 * {@link GlobalInitializer}. A parameter name 'links' contains a list of link
 * definitions.
 * 
 * The definition for 'nodes' is: <br/>
 * <b>nodeLabel1,nodeLabel2,...</b> <br/>
...
public class BasicPamInitializer implements Initializer {
	...
	private static final Logger logger = Logger
			.getLogger(BasicPamInitializer.class.getCanonicalName());
		...
		if (nodes != null) {
			...
			for (String nodeDef : defs) {
				...
				if ("".equals(label)) {
					...
					// PamNode node = (PamNode) factory.getNode("PamNodeImpl",
					// label);
					if (node == null) {
						...
						if (nodeParams.length > 1) {
							...
							try {
								blActivation = Double
										.parseDouble(nodeParams[2]);
							} catch (NumberFormatException e) {
								logger
										.log(
												Level.WARNING,
												"Bad base-level activation for link {1}.",
												new Object[] {
														TaskManager
																.getCurrentTick(),
														node });
		...
		if (links != null) {
			...
			for (String linkDef : linkDefs) {
				...
				if (linkParams.length < 2) {
					logger.log(Level.WARNING, "bad link specification "
							+ linkDef, TaskManager.getCurrentTick());
					continue;
				}
				Node source = pam.getNode(linkParams[0].trim());
				Node sink = pam.getNode(linkParams[1].trim());
				if (source != null && sink != null) {
					// Link link = factory.getLink("PamLinkImpl", source, sink,
					// PerceptualAssociativeMemoryImpl.PARENT);
					// pam.addDefaultLink(link);
					PamLink pl = pam.addDefaultLink(source, sink,
							PerceptualAssociativeMemoryImpl.PARENT);
					if (pl == null) {
						logger.log(Level.WARNING, "bad link specification "
								+ linkDef, TaskManager.getCurrentTick());
					...
					logger.log(Level.WARNING, "could not find source or sink "
							+ linkDef, TaskManager.getCurrentTick());
				}
