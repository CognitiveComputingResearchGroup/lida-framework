...
/**
 * Default <i> abstract </i> (i.e. must be overridden to be used) implementation
 * of the {@link SensoryMemory} module. This module should sense the
 * environment, store the sensed data and processing it. It should expect access
 * to its content from {@link DetectionAlgorithm}s via method
 * {@link SensoryMemory#getSensoryContent(String, Map)} and it may transmit
 * content to {@link SensoryMotorMemory}.
 * 
 * @author Ryan J. McCall
 */
public abstract class SensoryMemoryImpl extends FrameworkModuleImpl implements
		SensoryMemory {

	private static Logger logger = Logger.getLogger(SensoryMemoryImpl.class
			.getCanonicalName());

	/**
	 * The {@link SensoryMemoryListener} references associated with this module.
	 */
	protected List<SensoryMemoryListener> sensoryMemoryListeners = new ArrayList<SensoryMemoryListener>();
	 ...
	 */
	@Override
	public abstract Object getSensoryContent(String modality,
