...
public class BasicSensoryMotorMemoryInitializer implements Initializer {
	...
	private static final Logger logger = Logger
			.getLogger(BasicSensoryMotorMemoryInitializer.class
					.getCanonicalName());
	private static final GlobalInitializer initializer = GlobalInitializer
			.getInstance();
				...
				if (value instanceof String) {
					...
					logger
							.log(
									...
									"Parameter name: "
											+ key
											+ " started with smm.mapping. but did not contain a valid def");
					continue;
				}
				logger.log(Level.INFO,
						"loading smm action-algorithm mapping: {0}", smmDef);
				...
				if (elements.length == 2) {
					...
					if ("".equals(algorithmName)) {
						logger.log(Level.WARNING,
								"missing algorithm name for smm: {0}", smmDef);
						continue;
					}
